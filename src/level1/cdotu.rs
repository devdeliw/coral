...
use core::arch::aarch64::{
    vld1q_f32, vdupq_n_f32, vfmaq_f32, vaddvq_f32, vaddq_f32, vuzp1q_f32, vuzp2q_f32, vfmsq_f32
};
use crate::level1::assert_length_helpers::required_len_ok_cplx;


#[inline]
pub fn cdotu(n: usize, x: &[f32], incx: isize, y: &[f32], incy: isize) -> [f32; 2] {
    // quick return
    ...
    unsafe {
        ...
        let stepx: isize = 2isize * incx;  
        let stepy: isize = 2isize * incy;

        let mut ix: isize = if incx >= 0 { 0 } else { 2isize * (n as isize - 1) * (-incx) };
        let mut iy: isize = if incy >= 0 { 0 } else { 2isize * (n as isize - 1) * (-incy) };
