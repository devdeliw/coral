use core::arch::aarch64::{vld1q_f64, vdupq_n_f64, vfmaq_f64, vaddvq_f64};
...
pub fn ddotf(
    ...
    unsafe {
        let (x_ptr, x_contig): (*const f64, Option<Vec<f64>>) = if incx == 1 {
            ...
            let mut xb = Vec::<f64>::with_capacity(m);
            xb.set_len(m);
        ...
        while j + 4 <= n {
            ...
            while i + 8 <= m {
                let x0 = vld1q_f64(x_ptr.add(i + 0));
                let x1 = vld1q_f64(x_ptr.add(i + 2));
                let x2 = vld1q_f64(x_ptr.add(i + 4));
                let x3 = vld1q_f64(x_ptr.add(i + 6));
