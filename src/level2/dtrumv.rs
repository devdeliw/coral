...
//!
//! This function implements the BLAS [`crate::level2::dtrmv`] routine for **upper triangular** matrices,
//! computing the in-place product `x := op(A) * x`, where `op(A)` is either `A` or `A^T`
//!
//! [`dtrumv`] function is crate visible and is implemented via [`crate::level2::dtrmv`] routine. 
//!
//! # Arguments
...
fn compute_upper_block_notranspose( 
    ...
    unsafe { 
        for k in 0..buf_len { 
            ...
            single_add_and_scale_f64(buffer.as_mut_ptr(), column, k, scale); 
            if unit_diag { 
        ...
        unsafe { 
            ...
            while idx + nb <= n { 
                ...
                if col_tail < n { 
                    ...
                    daxpyf(nb, cols_left, &x[col_tail..], 1, mat_panel, lda, y_block, 1);
                }
        ...
        unsafe { 
            ...
            while idx >= nb { 
                ...
                if idx > 0 { 
                    ...
                    ddotf(rows_left, cols_left, mat_panel, lda, &x[..rows_left], 1, y_block);
                }
            }
...
pub(crate) fn dtrumv( 
    ...
    match transpose { 
        CoralTranspose::NoTranspose        => dtrumv_notranspose(n, unit_diag, matrix, lda, x, incx),
        CoralTranspose::Transpose          => dtrumv_transpose  (n, unit_diag, matrix, lda, x, incx),
