...
//! - `alpha`  ([f64; 2])        : Scalar multiplier applied to `A * x` (complex: `[re, im]`).
//! - `matrix` (&[f64])          : Input slice containing the matrix `A`; interleaved complex
//! - `lda`    (usize)           : Leading dimension of `A` in complex units.
//! - `x`      (&[f64])          : Input complex vector of length `n`, with stride `incx` in
//!                              | complex units (underlying f64 stride is `2 * incx`).
//! - `incx`   (usize)           : Stride between consecutive complex elements of `x`.
//! - `beta`   ([f64; 2])        : Scalar multiplier applied to `y` prior to accumulation.
//! - `y`      (&mut [f64])      : Input/output complex vector of length `n`, with stride `incy`
...
use crate::level1_special::zaxpyf::zaxpyf;
...
pub fn zhemv(
    ...
    pack_and_scale_c64(n, alpha, x, incx, &mut xbuffer); 
    ...
    if lda == n { 
        ...
        if packed_y { write_back_c64(n, &ybuffer, y, incy); }
        return; 
    } 
        ...
        match uplo { 
            CoralTriangular::UpperTriangular => { 
                ...
                { 
                    ...
                    let y_head: &mut [f64] = &mut y_slice[2 * row_idx .. 2 * (row_idx + mb_eff)]; 
                    for j in 0..mb_eff { 
                ...
                while col_idx < n {
                    ...
                    let y_head: &mut [f64]  = &mut y_pre[2 * row_idx .. 2 * (row_idx + mb_eff)]; 
                    let start_tail          = 2 * (col_idx - (row_idx + mb_eff));
                    let y_tail: &mut [f64]  = &mut y_post[start_tail .. start_tail + 2 * nb_eff];
            ...
            CoralTriangular::LowerTriangular => { 
                ...
                while col_idx < row_idx {
                    ...
                    let y_left: &mut [f64] = &mut y_left_region[2 * col_idx .. 2 * (col_idx + nb_eff)];
                    let y_head: &mut [f64] = &mut y_head_region[.. 2 * mb_eff];
