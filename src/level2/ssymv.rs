...
//! ```
//!
//! where `A` is an `n` × `n` **symmetric** column-major matrix. Only the triangle
...
//! it fuses a column-wise SAXPY stream with column-dots so each `A` element is read
...
pub fn ssymv(
    ...
    debug_assert!(required_len_ok(x.len(), n, incx), "x too short for n/incx (SYMV)");
    debug_assert!(required_len_ok(y.len(), n, incy), "y too short for n/incy (SYMV)");
    debug_assert!(
        required_len_ok_matrix(matrix.len(), n, n, lda),
        "matrix too short for given n and lda (SYMV)"
    );

    // y := beta * y 
    if beta != 0.0 { sscal(n, beta, y, incy); } 
    if alpha == 0.0 { return; } 
