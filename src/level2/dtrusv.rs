//! Performs a double precision triangular solve (TRSV) with an upper triangular matrix.
//!
//! This function implements the BLAS [`crate::level2::dtrsv`] routine for **upper triangular** matrices,
//! solving the system `op(A) * x = b` in place for `x`, where `op(A)` is either `A` or `A^T`.
//!
//! The [`dtrusv`] function is crate-visible and is implemented via 
...
fn update_tail_transpose( 
    ...
    unsafe { 
        ...
        // fused dot 
        ddotf(nb, rows_below, mat_view, lda, &x_block_neg[..nb], 1, x_tail_slice);
    }
}

#[inline] 
fn dtrusv_notranspose( 
    n           : usize, 
    unit_diag   : bool, 
    matrix      : &[f64], 
    lda         : usize, 
        ...
        unsafe { 
            ...
            while diag_idx != usize::MAX { 
                ...
                if diag_idx > 0 { 
                    ...
                    daxpyf(diag_idx, nb, &x_block_neg, 1, mat_panel, lda, y_head, 1);
                }
...
pub(crate) fn dtrusv(
    ...
    match transpose { 
        CoralTranspose::NoTranspose        => dtrusv_notranspose(n, unit_diag, matrix, lda, x, incx),
        CoralTranspose::Transpose          => dtrusv_transpose  (n, unit_diag, matrix, lda, x, incx),
        CoralTranspose::ConjugateTranspose => dtrusv_transpose  (n, unit_diag, matrix, lda, x, incx),
    }
}
