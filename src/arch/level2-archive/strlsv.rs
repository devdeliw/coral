...
#[inline(always)]
unsafe fn strlsv_unblk_scalar_trans(
    n         : usize,
    unit_diag : bool,
    a_ptr     : *const f32,
    inc_row_a : isize,
...
#[inline(always)]
unsafe fn gemv_t_small_k<const BF: usize>(
    rows   : usize,      
    bf     : usize,      
    a_pan  : *const f32, 
    lda    : usize,
        ...
        unsafe {
            ...
            while j + bf <= n {
                ...
                if tail < n {
                    ...
                    saxpyf(rows, bf, &neg_panel, 1, a_panel, lda, y_tail, 1);
                }
                j += bf;
            }
        ...
        unsafe {
            ...
            while j > 0 {
                ...
                if rows > 0 {
                    ...
                    gemv_t_small_k::<BF_F32>(rows, curr, a_panel, lda, x_tail, &mut acc);
                ...
                strlsv_unblk_tdiag(curr, unit_diag, ablk, lda, xblk);
            }
        }
    } else {
        unsafe {
            strlsv_unblk_scalar_trans(
                n, unit_diag, a.as_ptr(), inc_row_a, inc_col_a, x.as_mut_ptr(), incx,
            );
        }
    }
...
pub fn strlsv(
    ...
    match trans {
        Trans::NoTrans   => strlsv_notrans(n, unit_diag, a, inc_row_a, inc_col_a, x, incx),
        Trans::Trans     => strlsv_trans  (n, unit_diag, a, inc_row_a, inc_col_a, x, incx), 
        Trans::ConjTrans => strlsv_trans  (n, unit_diag, a, inc_row_a, inc_col_a, x, incx),
    }
}
