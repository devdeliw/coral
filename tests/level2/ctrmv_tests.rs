use blas_src as _;
use cblas_sys::{ CBLAS_LAYOUT, CBLAS_TRANSPOSE, CBLAS_DIAG, CBLAS_UPLO, cblas_ctrmv };
    ...
    for j in 0..n {
        for i in 0..=j {
            ...
            if i == j {
                ...
                a[idx]     = 1.0 + (0.02 as f32) * (i as f32);
                a[idx + 1] = 0.01 - (0.005 as f32) * (i as f32);
            } else {
                // small complex off-diagonal decaying with distance
                let d = (j - i) as f32;
                a[idx]     = (0.01 as f32) / (1.0 + d);
    ...
    for j in 0..n {
        for i in j..n {
            ...
            if i == j {
                // mildly varying diagonal
                a[idx]     = 1.0 + (0.02 as f32) * (i as f32);
                a[idx + 1] = -0.012 + (0.004 as f32) * (i as f32);
            } else {
                // small complex off-diagonal decaying with distance
                let d = (i - j) as f32;
                a[idx]     = (0.01 as f32) / (1.0 + d);
    ...
    for j in 0..n {
        for i in 0..=j {
            ...
            if i == j {
                ...
                a[idx]     = 2.5 + (0.05 as f32) * (i as f32);
                a[idx + 1] = -1.7 + (0.01 as f32) * (i as f32);
            } else {
                // small complex off-diagonal decaying with distance
                let d = (j - i) as f32;
...
fn make_lower_unit_c32(
    ...
    for j in 0..n {
        for i in j..n {
            ...
            if i == j {
                // ignored by unit diag
                a[idx]     = -2.0 + (0.04 as f32) * (i as f32);
                a[idx + 1] =  1.2 - (0.02 as f32) * (i as f32);
            } else {
                // small complex off-diagonal decaying with distance
                let d = (i - j) as f32;
...
fn run_case(
    ...
    let x0 = make_strided_vec_c32(
        ...
        incx,
        |k| (
            (0.1 as f32) + (0.2 as f32) * (k as f32), 
            (-0.05 as f32) + (0.03 as f32) * (k as f32)
        ),
    );
