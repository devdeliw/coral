use blas_src as _;
use cblas_sys::{ cblas_dger, CBLAS_LAYOUT };
use coral::level2::dger;
...
fn make_strided_vec(
    ...
    let mut v: Vec<f64> = vec![0.0; (len_logical - 1) * inc + 1];
...
fn run_quick_returns() {
    {
        ...
        let a0: Vec<f64> = vec![0.0; lda * n];
        let x            = make_strided_vec(1, 1, |_| 1.0);
        let y            = make_strided_vec(n, 1, |j| 0.2 + 0.1 * (j as f64));
...

    {
        ...
        let x  = make_strided_vec(m, 1, |i| 0.3 - 0.02 * (i as f64));
