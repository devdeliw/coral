...
fn run_all_ops(
    ...
    for &(alpha, beta) in cases {
        ...
        run_case(CoralTranspose::ConjugateTranspose, CoralTranspose::NoTranspose, m, n, k, lda_t, ldb_n, ldc, cases[0].0, cases[0].1);
        run_case(CoralTranspose::NoTranspose, CoralTranspose::ConjugateTranspose, m, n, k, lda_n, ldb_t, ldc, cases[1].0, cases[1].1);
    }
}
...
#[test]
fn not_block_multiple() {
    // sizes not multiples of MR, NR
    let (m, n, k) = (13, 14, 11);
    let lda_n = m;  
    let lda_t = k;  
    let ldb_n = k;  
