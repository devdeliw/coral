...
fn upper_small() {
    let n     = 7usize;
    let lda   = n;
    let alpha = [1.10f64, -0.35f64];

    let a0 = make_hermitian_col_major(n, lda);
    let x  = (0..n)
        .flat_map(|i| [0.2  + 0.10 * (i as f64), -0.05 + 0.02 * (i as f64)])
        .collect::<Vec<_>>();
    let y  = (0..n)
        .flat_map(|i| [-0.3 + 0.05 * (i as f64),  0.07 - 0.01 * (i as f64)])
        .collect::<Vec<_>>();
...
fn lower_small() {
    let n     = 7usize;
    let lda   = n;
    let alpha = [-0.80f64, 0.50f64];

    let a0 = make_hermitian_col_major(n, lda);
    let x  = (0..n)
        .flat_map(|i| [-0.2 + 0.07  * (i as f64), 0.06 - 0.01 * (i as f64)])
        .collect::<Vec<_>>();
    let y  = (0..n)
        .flat_map(|i| [ 0.4 - 0.055 * (i as f64), -0.03 + 0.02 * (i as f64)])
        .collect::<Vec<_>>();
...
fn large_upper() {
    let n     = 1024usize;
    let lda   = n;
    let alpha = [-0.37f64, 0.20f64];

    let a0 = make_hermitian_col_major(n, lda);
    let x  = (0..n)
        .flat_map(|i| [0.05 + 0.002 * (i as f64), -0.02 + 0.0007 * (i as f64)])
        .collect::<Vec<_>>();
    let y  = (0..n)
        .flat_map(|i| [0.40 - 0.003 * (i as f64),  0.01 - 0.0005 * (i as f64)])
        .collect::<Vec<_>>();
...
fn large_lower() {
    let n     = 512usize;
    let lda   = n;
    let alpha = [0.93f64, -0.10f64];

    let a0 = make_hermitian_col_major(n, lda);
    let x  = (0..n)
        .flat_map(|i| [-0.20 + 0.0015 * (i as f64), 0.03 - 0.0004 * (i as f64)])
        .collect::<Vec<_>>();
    let y  = (0..n)
        .flat_map(|i| [ 0.10 + 0.0020 * (i as f64), -0.01 + 0.0003 * (i as f64)])
        .collect::<Vec<_>>();
...
fn lower_padded_strided() {
    let n     = 9usize;
    let lda   = n + 3;
    let alpha = [-0.85f64, 0.25f64];

    let a0   = make_hermitian_col_major(n, lda);
    let incx = 2usize;
    let incy = 3usize;
    let x    = make_strided_cvec(n, incx, |i| [0.05 + 0.03 * (i as f64), -0.02 + 0.01 * (i as f64)]);
    let y    = make_strided_cvec(n, incy, |i| [0.40 - 0.02 * (i as f64),  0.03 - 0.01 * (i as f64)]);
...
fn accumulate_twice() {
    let n      = 64usize;
    let lda    = n;
    let alpha1 = [1.20f64, 0.30f64];
    let alpha2 = [-0.70f64, -0.20f64];
