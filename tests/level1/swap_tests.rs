...
#[test]
fn sswap_matches_cblas_unit_stride() {
    ...
    for &n in &sizes {
        ...
        unsafe {
            cblas_sswap(n as i32, x_blas.as_mut_ptr(), 1, y_blas.as_mut_ptr(), 1);
        }
...
#[test]
fn sswap_matches_cblas_nonunit_stride() {
    let n = 50usize;
    let incx = 2isize;
    let incy = 3isize;
    ...
    unsafe { cblas_sswap(n as i32, x_blas.as_mut_ptr(), incx as i32, y_blas.as_mut_ptr(), incy as i32); }
...
#[test]
fn dswap_matches_cblas_unit_stride() {
    let n = 129usize;
...
#[test]
fn dswap_matches_cblas_nonunit_stride() {
    let n = 64usize;
    let incx = 3isize;
    let incy = 2isize;
    ...
    unsafe { cblas_dswap(n as i32, x_blas.as_mut_ptr(), incx as i32, y_blas.as_mut_ptr(), incy as i32); }
...
#[test]
fn cswap_matches_cblas_unit_stride() {
    let n = 77usize;

    let mut x_pairs: Vec<[f32; 2]> = (0..n).map(|i| [i as f32 * 0.5, i as f32 * 0.5 + 0.1]).collect();
    ...
    let x_flat: &mut [f32] = unsafe { core::slice::from_raw_parts_mut(x_pairs.as_mut_ptr() as *mut f32, 2*n) };
    let y_flat: &mut [f32] = unsafe { core::slice::from_raw_parts_mut(y_pairs.as_mut_ptr() as *mut f32, 2*n) };

    cswap(n, x_flat, 1, y_flat, 1);
    unsafe { cblas_cswap(n as i32, x_pairs_blas.as_mut_ptr(), 1, y_pairs_blas.as_mut_ptr(), 1); }

    let x_blas_flat: &[f32] = unsafe { core::slice::from_raw_parts(x_pairs_blas.as_ptr() as *const f32, 2*n) };
    let y_blas_flat: &[f32] = unsafe { core::slice::from_raw_parts(y_pairs_blas.as_ptr() as *const f32, 2*n) };
    assert_bits_eq_f32(x_flat, x_blas_flat);
    assert_bits_eq_f32(y_flat, y_blas_flat);
}

#[test]
fn cswap_matches_cblas_nonunit_stride() {
    let n = 60usize;
    let inc = 2isize;
    ...
    let xf: &mut [f32] = unsafe { core::slice::from_raw_parts_mut(x_pairs.as_mut_ptr() as *mut f32, 2*len) };
    let yf: &mut [f32] = unsafe { core::slice::from_raw_parts_mut(y_pairs.as_mut_ptr() as *mut f32, 2*len) };

    cswap(n, xf, inc, yf, inc);
    unsafe { cblas_cswap(n as i32, xb.as_mut_ptr(), inc as i32, yb.as_mut_ptr(), inc as i32); }

    let xb_flat: &[f32] = unsafe { core::slice::from_raw_parts(xb.as_ptr() as *const f32, 2*len) };
    let yb_flat: &[f32] = unsafe { core::slice::from_raw_parts(yb.as_ptr() as *const f32, 2*len) };
    assert_bits_eq_f32(xf, xb_flat);
    assert_bits_eq_f32(yf, yb_flat);
}

#[test]
fn zswap_matches_cblas_unit_stride() {
    let n = 66usize;

    let mut x_pairs: Vec<[f64; 2]> = (0..n).map(|i| [i as f64 * 0.8, i as f64 * 0.8 + 0.2]).collect();
    let mut y_pairs: Vec<[f64; 2]> = (0..n).map(|i| [-(i as f64) * 0.5, -(i as f64) * 0.5 + 0.4]).collect();
    let mut xb = x_pairs.clone();
    let mut yb = y_pairs.clone();

    let xf: &mut [f64] = unsafe { core::slice::from_raw_parts_mut(x_pairs.as_mut_ptr() as *mut f64, 2*n) };
    let yf: &mut [f64] = unsafe { core::slice::from_raw_parts_mut(y_pairs.as_mut_ptr() as *mut f64, 2*n) };

    zswap(n, xf, 1, yf, 1);
    unsafe { cblas_zswap(n as i32, xb.as_mut_ptr(), 1, yb.as_mut_ptr(), 1); }
...
#[test]
fn zswap_matches_cblas_nonunit_stride() {
    let n = 48usize;
    let inc = 3isize;
    let step = inc as usize;
    let len = 1 + (n - 1) * step;
    ...
    let xf: &mut [f64] = unsafe { core::slice::from_raw_parts_mut(x_pairs.as_mut_ptr() as *mut f64, 2*len) };
    let yf: &mut [f64] = unsafe { core::slice::from_raw_parts_mut(y_pairs.as_mut_ptr() as *mut f64, 2*len) };

    zswap(n, xf, inc, yf, inc);
    unsafe { cblas_zswap(n as i32, xb.as_mut_ptr(), inc as i32, yb.as_mut_ptr(), inc as i32); }

    let xb_flat: &[f64] = unsafe { core::slice::from_raw_parts(xb.as_ptr() as *const f64, 2*len) };
    let yb_flat: &[f64] = unsafe { core::slice::from_raw_parts(yb.as_ptr() as *const f64, 2*len) };
    assert_bits_eq_f64(xf, xb_flat);
    assert_bits_eq_f64(yf, yb_flat);
}
