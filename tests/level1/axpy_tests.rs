...
fn caxpy_matches_cblas() {
    ...
    let xf_after: &[f32] = unsafe {
        core::slice::from_raw_parts(x_pairs.as_ptr() as *const f32, 2*n)
    };

    assert_bits_eq_f32(xf_after, unsafe { 
...
fn caxpy_matches_cblas_stride() {
    ...
    let xf: &[f32]     = unsafe {
        core::slice::from_raw_parts(x_pairs.as_ptr() as *const f32, 2*len_x)
    };
    let yf: &mut [f32] = unsafe {
        core::slice::from_raw_parts_mut(y_pairs.as_mut_ptr() as *mut f32, 2*len_y) 
    };
    ...
    let xb_flat: &[f32] = unsafe {
        core::slice::from_raw_parts(xb.as_ptr() as *const f32, 2*len_x)
    };
    let yb_flat: &[f32] = unsafe {
        core::slice::from_raw_parts(yb.as_ptr() as *const f32, 2*len_y) 
    };
...
fn zaxpy_matches_cblas_stride() {
    ...
    let xf: &[f64] = unsafe { 
        core::slice::from_raw_parts(x_pairs.as_ptr() as *const f64, 2*len_x)
    };
    let yf: &mut [f64] = unsafe { 
        core::slice::from_raw_parts_mut(y_pairs.as_mut_ptr() as *mut f64, 2*len_y)
    };
    ...
    let xb_flat: &[f64] = unsafe { 
        core::slice::from_raw_parts(xb.as_ptr() as *const f64, 2*len_x)
    };
    let yb_flat: &[f64] = unsafe { 
        core::slice::from_raw_parts(yb.as_ptr() as *const f64, 2*len_y)
    };
