...
pub fn bench_zgemm_cc_fixed(c: &mut Criterion) {
    let n: usize = 1024;
    let (m, k) = (n, n);
    ...
    c.bench_function("coral_zgemm_cc", |bch| {
    ...
    c.bench_function("blas_zgemm_cc", |bch| {
        bch.iter_batched_ref(
            || c0.clone(),
            |c_buf| unsafe {
                cblas_zgemm(
...
pub fn bench_zgemm_cc_sweep(c: &mut Criterion) {
    let sizes: Vec<usize> = (128..=2048).step_by(128).collect();
    let mut group = c.benchmark_group("zgemm_cc");
...
criterion_group!(benches, bench_zgemm_cc_fixed, bench_zgemm_cc_sweep);
