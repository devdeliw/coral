...
use coral_safe::types::{VectorMut, VectorRef, MatrixRef, CoralTranspose}; 
use coral_safe::level2::sgemv; 
...
#[test]
fn unit_stride_n_lda() -> CoralResult {
    let m = 1024;
    let n = 512;
    let incx = 1;
    let incy = 1;
...
#[test]
fn unit_stride_t_lda() -> CoralResult {
    let m = 1024;
    let n = 512;
    let incx = 1;
    let incy = 1;
