...
pub fn bench_zgemm_nn_fixed(c: &mut Criterion) {
    let n: usize = 1024;
    let (m, k) = (n, n);
    ...
    let alpha: [f64; 2] = [1.000123, 0.0];
    let beta:  [f64; 2] = [0.000321, 0.0];
    ...
    c.bench_function("coral_zgemm_nn", |bch| {
    ...
    c.bench_function("blas_zgemm_nn", |bch| {
        bch.iter_batched_ref(
            || c0.clone(),
            |c_buf| unsafe {
                cblas_zgemm(
...
pub fn bench_zgemm_nn_sweep(c: &mut Criterion) {
    let sizes: Vec<usize> = (128..=2048).step_by(128).collect();

    let mut group = c.benchmark_group("zgemm_nn");
...
criterion_group!(benches, bench_zgemm_nn_fixed, bench_zgemm_nn_sweep);
