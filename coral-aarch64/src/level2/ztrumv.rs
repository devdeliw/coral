//! Performs a double precision complex triangular matrix–vector multiply (ZTRMV) with 
//! an upper triangular matrix.
//!
//! This function implements the BLAS [`crate::level2::ztrmv`] routine for **upper triangular** matrices,
...
//! - `matrix`     (&[f64])          : Input slice of interleaved upper triangular complex matrix `A`.
//! - `lda`        (usize)           : Leading dimension of `A`.
//! - `x`          (&mut [f64])      : Input/output slice containing the complex vector `x`.
...
//! - Fused level-1 routines ([`zaxpyf`], [`zdotcf`], and [`zdotuf`]) are used for panel updates.
...
/// - `y_block`     (*mut f64)   : Pointer to the output subvector; overwrites `x_block`.
#[inline(always)]
        ...
        unsafe { 
            ...
            while idx + nb <= n { 
                ...
                if col_tail < n { 
                    ...
                    zaxpyf(nb, cols_left, &x[2 * col_tail..], 1, mat_panel, lda, y_block, 1);
                }
        ...
        unsafe { 
            ...
            while idx >= nb { 
                ...
                if idx > 0 { 
                    ...
                    zdotuf(rows_left, cols_left, mat_panel, lda, &x[..2 * rows_left], 1, y_block);
                }
            }
        ...
        unsafe { 
            ...
            while idx >= nb { 
                ...
                if idx > 0 { 
                    ...
                    zdotcf(rows_left, cols_left, mat_panel, lda, &x[..2 * rows_left], 1, y_block);
                }
            }
...
pub(crate) fn ztrumv( 
    ...
    match transpose { 
        CoralTranspose::NoTranspose        => ztrumv_notranspose        (n, unit_diag, matrix, lda, x, incx),
        CoralTranspose::Transpose          => ztrumv_transpose          (n, unit_diag, matrix, lda, x, incx),
        CoralTranspose::ConjugateTranspose => ztrumv_conjugatetranspose (n, unit_diag, matrix, lda, x, incx),
    }
}
