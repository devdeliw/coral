...
pub(crate) fn zgemv_notranspose(
    n_rows  : usize,
    n_cols  : usize,
    alpha   : [f64; 2],
    matrix  : &[f64],   
    ...
    if !(beta[0] == 1.0 && beta[1] == 0.0) {
        if beta[1] == 0.0 {
            zdscal(n_rows, beta[0], y, incy);
        } else {
            zscal(n_rows, beta, y, incy);
        }
    }
    ...
    pack_and_scale_c64(n_cols, alpha, x, incx, &mut xbuffer);
    ...
    let y_slice: &mut [f64] = if incy == 1 { y } else { 
        packed_y = true; 
        pack_c64(n_rows, y, incy, &mut ybuffer); 
    ...
    if lda == n_rows { 
        unsafe { 
            ...
            zaxpyf(n_rows, n_cols, &xbuffer, 1, matrix_view, lda, y_slice, 1);
        }
    } else {
        // general case  
        //  
        ...
        while row_idx < n_rows {
            ...
            let y_sub: &mut [f64] = &mut y_slice[2 * row_idx .. 2 * (row_idx + mb_eff)];
    ...
    if packed_y { 
        write_back_c64(n_rows, &ybuffer, y, incy);
    }
}
